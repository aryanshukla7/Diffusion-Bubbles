...
def create_vtk_for_positive_negative(data, vtk_file_path):
    ...
    for idx, row in data.iterrows():
        if row['InfoFlag'] == 0:          # Positive feature
            ...
            continue # Skip other InfoFlag values (reorientated features)
...
def run_processing(base_path, key_subdir, vtk_subdir):
    base_path = os.path.expanduser(base_path)
    key_dir = os.path.join(base_path, key_subdir)
    vtk_dir_base = os.path.join(base_path, vtk_subdir)
...
def main():
    parser = argparse.ArgumentParser(
        description="Process molecule key files and generate VTK outputs."
    )
    parser.add_argument(
        "--base_path", 
        type=str, 
        default="~/Desktop/Experiments/molecules/data",
        help="Base directory where molecule data is located."
    )
    parser.add_argument(
        "--key_subdir",
        type=str,
        default="molecules-key",
        help="Subdirectory (relative to base_path) containing .key files."
    )
    parser.add_argument(
        "--vtk_subdir",
        type=str,
        default="molecules-vtk",
        help="Subdirectory (relative to base_path) where VTK output will be saved."
    )
    args = parser.parse_args()
    run_processing(args.base_path, args.key_subdir, args.vtk_subdir)
